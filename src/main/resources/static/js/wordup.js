...
function addNewWordSubmission(word) {
    ...
    for (var key in model.wordSubmissions) {
        if (model.wordSubmissions[key].word == word) {
            alreadyUsed = true;
        }
    }
...
function checkIfWordIsReal(word) {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            for (var key in model.wordSubmissions) {
                if (model.wordSubmissions[key].word == word) {
                    model.wordSubmissions[key].isRealWord = theAnswer;
                    console.log(key + " -> " + model.wordSubmissions[key].word);
                    console.log(key + " -> " + model.wordSubmissions[key].isRealWord);
                }
            }
...
function isDisallowedLetter(letter) {
    ...
    if ($.inArray(letter, model.allowedLetters) >= 0){
        return false;
    }
    return true;
}
...
function generateAllowedLetters() {
    return chooseN(7, Object.keys(scrabblePointsForEachLetter), Object.keys(vowels));
}
 ...
 */
 ...
 var U = null;
var vowel = null;
function chooseN(n, items, itemvowels) {
    var selectedItems = [];
    var vowelList = Object.keys(vowels);
    vowel = 0;
    Q = 0;
    U = 0;
    ...
    for (var i = 0; i < total; i++) {
        ...
        if (vowelList.indexOf(items[index]) >= 0){
            vowel++;
            console.log("1");
            if (items[index] == "u"){
                U++;
        ...
        if(i == total-1){
            if(Q > 0 && U == 0){
                selectedItems.push(items[items.indexOf("u")]);
                console.log("4");
            }
            else if(vowel == 0){
                index = Math.floor(Math.random() * itemvowels.length);
                selectedItems.push(itemvowels[index]);
                console.log("5");
            }
            else{
                items.splice(items.indexOf("q"), 1);
                index = Math.floor(Math.random() * items.length);
